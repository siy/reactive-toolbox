...
public interface Submitter {
     ...
     */
    Submitter splice(final FileHandle fdIn, final long offsetIn,
                     final FileHandle fdOut, final long offsetOut,
                     final int count, final EnumSet<SpliceFlags> flags,
                     final Consumer<NativeError> completionHandler,
                     final Option<Timeout> timeout);

    /**
     * Same as {@link #splice(FileHandle, long, FileHandle, long, int, EnumSet, Consumer, Option)} except
     * no timeout is specified.
     */
    default Submitter splice(final FileHandle fdIn, final long offsetIn,
                             final FileHandle fdOut, final long offsetOut,
                             final int count, final EnumSet<SpliceFlags> flags,
                             final Consumer<NativeError> completionHandler) {
     ...
     *         Optional operation timeout.
     */
    Submitter read(final FileHandle fdIn,
    ...
    /**
     * Same as {@link #read(FileHandle, ByteBuffer, long, Consumer, Option)} except no timeout is specified.
     */
    default Submitter read(final FileHandle fdIn,
                           final ByteBuffer buffer,
                           final long offset,
                           final Consumer<NativeError> completionHandler) {
    ...
    /**
     * Same as {@link #read(FileHandle, ByteBuffer, long, Consumer, Option)} except no offset is specified.
    ...
    /**
     * Same as {@link #read(FileHandle, ByteBuffer, long, Consumer, Option)} except no offset and no timeout is specified.
     * Convenient for using with sockets or reading file at current position.
     */
    default Submitter read(final FileHandle fdIn,
                           final ByteBuffer buffer,
     ...
     *         Optional operation timeout.
     */
    Submitter write(final FileHandle fdOut,
                    final ByteBuffer buffer,
                    final long offset,
                    final Consumer<NativeError> completionHandler,
                    final Option<Timeout> timeout);
    ...
    /**
     * Same as {@link #write(FileHandle, ByteBuffer, long, Consumer, Option)} except no timeout is specified.
     */
    default Submitter write(final FileHandle fdOut,
                            final ByteBuffer buffer,
                            final long offset,
                            final Consumer<NativeError> completionHandler) {
    ...
    /**
     * Same as {@link #write(FileHandle, ByteBuffer, long, Consumer, Option)} except no offset is specified.
    ...
    /**
     * Same as {@link #write(FileHandle, ByteBuffer, long, Consumer, Option)} except no offset and no timeout is specified.
     * Convenient for using with sockets or writing file at current position.
     */
    default Submitter write(final FileHandle fdOut,
                            final ByteBuffer buffer,
     ...
     *         Callback which will be invoked on completion.
     */
    Submitter close(final FileHandle fd, Option<Timeout> timeout, final Consumer<NativeError> completionHandler);

    /**
     * Same as {@link #close(FileHandle, Option)} except no timeout is specified.
     */
    default Submitter close(final FileHandle fd, final Consumer<NativeError> completionHandler) {
    ...
    Submitter open(final Path path,
                   ...
                   final Consumer<Result<FileHandle>> completionHandler);
     ...
     *         Callback which will be invoked on completion.
     */
    Submitter clientSocket(final EnumSet<SocketOpenFlags> flags, final Consumer<Result<FileHandle>> completionHandler);
     ...
     *         Callback which will be invoked on completion.
     */
    Submitter serverSocket(final EnumSet<SocketOpenFlags> flags,
                           final int port,
                           final Option<InetAddress> address,
                           final int backLog,
                           final Consumer<Result<FileHandle>> completionHandler);

    enum SocketOpenFlags implements Bitmask {
        STREAM(1 << 0),
        NONBLOCK(1 << 1),
        REUSE_ADDR(1 << 2),
        REUSE_PORT(1 << 3);
        ...
        SocketOpenFlags(final int mask) {
            this.mask = mask;
        }
     ...
     *         Callback which will be invoked on completion.
     */
    Submitter accept(final FileHandle socket, final EnumSet<SocketAcceptFlags> flags, final Consumer<Result<ClientConnection>> completionHandler);

    enum SocketAcceptFlags implements Bitmask {
        SOCK_CLOEXEC(02000000),     /* Set close-on-exec flag for the descriptor */
        SOCK_NONBLOCK(00004000);    /* Mark descriptor as non-blocking */

        private final int mask;

        SocketAcceptFlags(final int mask) {
            this.mask = mask;
        }
    ...
    record ClientConnection(FileHandle handle, InetAddress address) {}
     ...
     *         Callback which will be invoked on completion.
     */
    Submitter connect(final FileHandle socket, final InetAddress address, final Consumer<NativeError> completionHandler);
