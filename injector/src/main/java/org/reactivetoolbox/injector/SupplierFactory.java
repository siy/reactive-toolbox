...
import org.reactivetoolbox.injector.annotations.PostConstruct;
...
public final class SupplierFactory {
    ...
    public static <T> Supplier<T> createInstanceSupplier(Constructor<T> constructor, List<Supplier<?>> parameters) {
        ...
        return tryBuildSingleton(factory, constructor.getDeclaringClass().getAnnotation(Singleton.class));
    }
    ...
    public static <T> Supplier<T> createMethodSupplier(Method method, List<Supplier<?>> parameters) {
        ...
        return tryBuildSingleton(supplier, method.getAnnotation(Singleton.class));
    }
    ...
    private static <T> Supplier<T> tryBuildSingleton(Supplier<T> instanceSupplier, Singleton singleton) {
        return Optional.ofNullable(singleton)
                       .map(s -> Suppliers.singleton(instanceSupplier,
